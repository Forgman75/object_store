...
- **`BaseInfo`** — общий предок для классов с названием и описанием (`Category`, `Order`)
- **`BaseProduct`** — общий предок для всех товаров

### Миксины
- **`LogCreationMixin`** — при создании объекта выводит в консоль имя класса и все переданные параметры
...
- **`Order`** — класс заказа на один товар с автоматическим расчётом итоговой стоимости
...
### Автоматическое логирование создания объектов (LogCreationMixin)

Миксин LogCreationMixin автоматически выводит в консоль информацию о каждом создаваемом объекте: имя класса и все переданные параметры. Формат вывода соответствует рекомендациям по реализации __repr__ — строки в кавычках, числа без.

```
# При создании объекта миксин автоматически печатает лог
phone = Smartphone(
    "iPhone 15", "Чёрный", 100000.0, 5,
    "Высокая", "15 Pro", 256, "Чёрный"
)
# Вывод в консоль:
# Smartphone('iPhone 15', 'Чёрный', 100000.0, 5, 'Высокая', '15 Pro', 256, 'Чёрный')
...
# Вывод в консоль:
# LawnGrass('Газонная трава', 'Зелёная', 500.0, 20, 'Россия', 7, 'Тёмно-зелёный')
```
Миксин встроен в цепочку наследования класса Product и работает для всех его наследников (Smartphone, LawnGrass) без дополнительной настройки.

### Создание заказов (Order)

Класс Order представляет заказ на один товар с указанием количества. Итоговая стоимость рассчитывается автоматически через свойство total_amount. Класс наследуется от BaseInfo, поэтому имеет общие атрибуты name и description с Category.
...
# Создаём заказ (имя и описание сформируются автоматически)
order = Order(phone, 2)

print(order.name)           # Заказ на iPhone 15
print(order.description)    # Покупка товара 'iPhone 15' в количестве 2 шт.
print(order.total_amount)   # 200000.0
print(order)                # Заказ на iPhone 15: iPhone 15 x 2 шт. = 200000.0 руб.

# Можно задать свои имя и описание
order2 = Order(
    phone, 3,
    name="Заказ №42",
    description="Подарок другу"
)
print(order2.name)          # Заказ №42
print(order2.total_amount)  # 300000.0

# Итоговая стоимость пересчитывается при изменении цены товара
phone.price = 120000.0
print(order.total_amount)   # 240000.0 (автоматический пересчёт)

# Валидация: нельзя создать заказ с не-товаром или отрицательным количеством
try:
    bad_order = Order("не товар", 5)
except TypeError as e:
    print(e)
    # В заказе может быть указан только товар класса Product или его наследников

try:
    bad_order = Order(phone, -1)
